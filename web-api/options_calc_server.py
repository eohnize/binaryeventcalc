...
def _extract_recent_earnings_reactions(ticker_obj, limit: int = 5) -> list[dict]:
    try:
        frame = None
        if hasattr(ticker_obj, "get_earnings_dates"):
            frame = ticker_obj.get_earnings_dates(limit=max(limit + 3, 8))
        if frame is None or frame.empty:
            frame = getattr(ticker_obj, "earnings_dates", None)
        if frame is None or frame.empty:
            return []
        ...
        hist = history.dropna(subset=["Close"]).copy()
        if hist.empty:
            return []

        hist["session_date"] = pd.to_datetime(hist.index, errors="coerce", utc=True).tz_convert(None).normalize()
        hist = hist.dropna(subset=["session_date"])
        if hist.empty:
            return []

        earnings = frame.reset_index().copy()
        raw_date_col = earnings.columns[0]
        earnings["earnings_date"] = pd.to_datetime(earnings[raw_date_col], errors="coerce", utc=True).dt.tz_convert(None).dt.normalize()
        earnings = earnings.dropna(subset=["earnings_date"]).sort_values("earnings_date", ascending=False)

        results: list[dict] = []
        for _, row in earnings.iterrows():
            event_date = row["earnings_date"]
            prev_close_series = hist.loc[hist["session_date"] < event_date, "Close"]
            next_close_series = hist.loc[hist["session_date"] > event_date, "Close"]
            if prev_close_series.empty or next_close_series.empty:
                continue

            prev_close = float(prev_close_series.iloc[-1])
            next_close = float(next_close_series.iloc[0])
            reaction_pct = ((next_close / prev_close) - 1.0) * 100 if prev_close > 0 else None

            reported = _safe_float(row.get("Reported EPS"))
            estimate = _safe_float(row.get("EPS Estimate"))
            surprise_pct = _safe_float(row.get("Surprise(%)"))
            if surprise_pct is None and reported is not None and estimate not in (None, 0):
                surprise_pct = ((reported - estimate) / abs(estimate)) * 100

            if surprise_pct is not None and surprise_pct > 0 and reaction_pct is not None and reaction_pct > 1:
                rating = "W"
                summary = "Beat + strong reaction"
            elif surprise_pct is not None and surprise_pct < 0:
                rating = "L"
                summary = "Miss / soft reaction"
            elif reaction_pct is not None and reaction_pct < -1:
                rating = "L"
                summary = "Negative post-earnings reaction"
            else:
                rating = "D"
                summary = "Muted reaction"
            ...
            results.append(
                {
                    ...
                    "reaction_pct": round(float(reaction_pct), 1) if reaction_pct is not None else None,
                }
            )
...
def catalyst(ticker: str):
    try:
        ...
        recent_earnings = _extract_recent_earnings_reactions(t)
